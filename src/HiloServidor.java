...
{
	private String directorioServer = "E:\\Servidor\\"; //Direccion est�tica del servidor
	...
	private String password = ""; //La contrase�a
	private Socket socket; //El socket
	private ArrayList<Socket> lista; //El arreglo de sockets para soporte de m�ltiples usuarios
	...
	public void run() 
	{
		//la sintaxis de mensajes ser� comando-^-param 1,param 2...param n - 1,param n
		//el primer mensaje ser� #password@usuario
		//el mensaje para crear ser� ^CREATE-^-password@usuario
		for(;;)
		{
			...
			if(mensajeEntrada.charAt(0) == '#')
			{
				...
				try
				{
					...
					//Se lee la contrase�a del archivo
					String pass = buferLectura.readLine();
					buferLectura.close();// Se cierra el archivo
					//se comparan la contrase�a del archivo con la escrita por el uusario
					if(pass.equals(password))
					{
						//Si es correcta la contrase�a se carga como directorio actual el directorio del usuario
						directorioActual = directorioUsuario;
						//se llama el m�todo LS
						LS();
					}
					else
					{
						//Se envia un mensaje de contrase�a incorrecta
						MESSAGEOUT("#BADPASS");
					}
			...
			else
			{
				...
				{
					//se separan los parametros para ser procesados en el m�todo
					StringTokenizer ST2 = new StringTokenizer(ST.nextToken(), ",");
					int limite = ST2.countTokens();
					String[] parametros = new String[limite];
					for(int i = 0; i < limite; i++)
	...
	//M�todo para crear un usuario nuevo y su directorio
	private void CREATE(String[] params)
	{
		//se obtienen el usuario y la contrase�a de los par�metros
		...
		try
		{
			...
			dir.mkdir();
			//Se asignan los directorios de usuario y el actual para la navegaci�n 
			directorioUsuario = directorioServer + usu + "\\";
			directorioActual = directorioUsuario;
			//Se crea el archivo donde se guardar� la contrase�a
			File archivoPass = new File(directorioUsuario + usu + ".pass");
			archivoPass.createNewFile();
			//Se abre el archivo y se escribie la contrase�a
			...
			escArchivo.close();
			//Se env�a un mensaje de nuevo usuario creado al cliente
	...
	private void MESSAGEOUT(String mensaje)
	{
		try
		{
			//se asigna una salida por el socket y se env�a el mensaje
	...
	private void CLOSE()
	{
		try
		{
			//Se env�a un mensaje de cierre de sesi�n al cliente
	...
	private void GET(String param)
	{
		try
		{
			...
			//se genera un bufer con tama�o de 4k
			...
			//se env�an las partes del archivo mientras hasta que se llega al final del archivo
	...
	private void PUT(String[] params)
	{
		try
		{
			...
			//Se obtiene el tama�o del archivo local
			long longArchivoLocal = Long.parseLong(params[1]);
			//se genera una variable para el tama�o del archivo remoto
			...
			//se env�a mensaje de que el servidor est� listo para recibir el archivo
			MESSAGEOUT("^PUTREADY");
			System.out.println("Servidor listo para recibir archivo del usuario: " + usuario);
			//se genera la entrada de bytes a trav�s del socket
			InputStream entrada = socket.getInputStream();
			//el ciclo continua mientras hasta que el tama�o enviado como par�metro coincida con el del archivo del servidor
			while(longArchivoLocal > longArchivoRemoto)
			{
				//se obtiene la cantidad de bytes
				cantBytes = entrada.read(bufer);
				//se suman los bytes acumulados en el env�o
				longArchivoRemoto += cantBytes;
				if(cantBytes < 0)
				{
					MESSAGEOUT("^EOF");
					System.out.println("Se lleg� al fin del archivo para el usuario: " + usuario);
				}
				if(cantBytes > 0)
			...
			//se llaman el m�todo para mostrar los archivos y directorios del servidor
			LS();
	...
	//M�todo para que el servidor sepa el directorio del cliente
	private void LCD(String[] params)
	{
		try
		{
			...
			if(carpeta.equals(".."))
			{
				...
				String newdir = "";
				//se vuelve a unir omitiendo el �ltimo
	...
	//M�todo para la navegaci�n del directorio del servidor
	private void CD(String dir)
	{
		try
		{
			...
			if(carpeta.equals(".."))
			{
				...
				if(!directorioActual.equals(directorioUsuario))
				{
					...
					LS();
					System.out.println("El usuario " + usuario + " se movi� a " + directorioActual);					
				}
			}
			else
			{
				...
				// y se env�a la nueva lista de archivos al cliente
				if(temp.isDirectory())
				{
		...
		catch(Exception e)
		{
			e.printStackTrace();
			MESSAGEOUT("#NODIR");
			System.out.println("El usuario " + usuario + " se movi� a un directorio inexistente.");
		}
	}
	//M�todo para enviar el listado de archivos dentro de la carpeta actual a el cliente
	private void LS()
	{
		try
		{
			...
			//se env�a un mensaje para decirle al cliente que limpie su listado actual
			MESSAGEOUT("^CLEARLIST");
			//se env�an los nombres de los archivos y directorios
	...
	//M�todo para borrar un archivo del servidor
	...
	//M�todo para crear un directorio en el servidor
	private void MKDIR(String param)
	{
		String dir = param;
		try
		{
			//se crea un archivo con el nombre enviado como par�metro
	...
	//M�todo para remover un directorio del servidor
	...
	//M�todo para enviar el directorio actual en el que se encuentra el servidor
	private void PWD()
	{
		//
